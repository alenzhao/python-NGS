...
def printpile(locus,refsequence,last,start,RQ,lenRQ,pileupfile):
...
#			print locus,k+1,refsequence[0][k],coverage #,bases,qvalues,mvalues,positions,mismatches,idlist;
			...
			for f in xrange(coverage):
				if bases[f] == 'I': Ic +=1;
			print >>pileupfile, locus,k+1,refsequence[0][k],coverage-Ic,'@'+bases,'@'+qvalues,'@'+mvalues,positions,mismatches,indelbases;
			RQ.popleft(); lenRQ -=1;
		else: 
			print >>pileupfile, locus,k+1,refsequence[0][k],0;
	return lenRQ;


def generate_pileup(readfile,sequences,reflist):
	current =0 ; index = sequences[reflist[current]]; last =0;
	pileupfile = open(readfile + '.pileup.temp','w');
...
#			if line[k][0:2] == 'XM': mismatches = int(line[k].split(':')[2]); break;
		...
		while locus != reflist[current]:
			lenRQ = printpile(reflist[current],index,last,index[1],RQ,lenRQ,pileupfile); 
			last = 0;
			current +=1;
		...
		lenRQ = printpile(locus,refsequence,last,start-1,RQ,lenRQ,pileupfile); 
		last = start-1;
...
def samtopileup(readfile,seqfile,targetfile):
	#http://docs.python.org/faq/library.html#threads
	...
	if ',' in readfile:
		...
		for readfile1 in readfilelist:
			t = Thread(target=generate_pileup, args=(readfile1,sequences,reflist,));
			t.start();
	else:
		generate_pileup(readfile,sequences,reflist);
	ticks2 = time.time();
	print >>sys.stderr, "time taken for processing files is",ticks2-ticks1;
