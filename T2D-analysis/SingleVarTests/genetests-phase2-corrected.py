...
def read_phase3_VCF(VCFfile):
	...
	for line in File: 
		if line[0] == '#' and line[1] == '#': continue
			...
			EO_counts_2 = [0,0]; LO_counts_2 = [0,0]; EO_counts_1 = [0,0]; LO_counts_1 = [0,0]; controls_counts_1 = [0,0]; 
			for j in xrange(9,len(var)): 
				geno = var[j].split(':'); 
				if geno[0] == '.': continue;
				counts = [int(geno[0].split(',')[0]),int(geno[0].split(',')[1])];
				if 'EO_phase2' in samplelist[j]: EO_counts_2[0] += counts[0]; EO_counts_2[1] += counts[1];
				elif 'phase2' in samplelist[j]: LO_counts_2[0] += counts[0]; LO_counts_2[1] += counts[1]; 
			for allele in alt: varlist[(chrom,position,ref,allele)]  = [EO_counts_2[0],EO_counts_2[1], LO_counts_2[0],LO_counts_2[1]];
			variants +=1; 
	File.close();
