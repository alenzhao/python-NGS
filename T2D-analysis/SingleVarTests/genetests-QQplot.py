...
sys.path.append("/home/vbansal/PROGRAMS/JOINTCODE-coral/PYTHON-scripts/NGS-library-python/");
...
def calculate_FETpvalues_allelecounts(vcffile,poolstatustable,poolsize):
	...
	for line in File:
		...
		casepools=0; controlpools=0;
		for i in xrange(samples):
			if poolDX_random[i][0] == -1: continue;  # ignore pool for case control analysis 
			try: 
				...
				if poolDX_random[i][0] == 0: H0 += poolsize; H1 += MLAC; H2 += MLAC2;
				elif poolDX_random[i][0] >= 1:  D0 +=poolsize; D1 += MLAC; D2 += MLAC2;
				if poolDX_random[i][0] == 2: E0 += poolsize; E1 += MLAC; E2 += MLAC2;
				if len(poolDX_random[i]) >=2 and poolDX_random[i][1] == 3: DC0 += poolsize; DC1 += MLAC; DC2 += MLAC2; 

			except IndexError: print 'Exception',i,samples,variant; 

		nopvalue_calc = 0;
		if (H1 + D1 >= 5): pvalue = fet(int(round(H1,0)),int(round(H0,0)),int(round(D1,0)),int(round(D0,0)));
		else: pvalue = [0,0,0]; nopvalue_calc = 1;

		## calculate p-values between early onset and controls
		if (H1+E1 >=5): pvalue1 = fet(int(round(H1,0)),int(round(H0,0)),int(round(E1,0)),int(round(E0,0)));
		else: pvalue1 = [0,0,0]; 

		## calculate p-value between early onset and late-onset
		if (D1+E1 >=5): pvalue2 = fet(int(round(D1,0)),int(round(D0,0)),int(round(E1,0)),int(round(E0,0)));
		else: pvalue2 = [0,0,0]; 
