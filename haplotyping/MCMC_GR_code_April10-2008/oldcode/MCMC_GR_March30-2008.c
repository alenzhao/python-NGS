...
// this code contains method for estimating switch error rate and was used for creating the SWitch error
// rates for GR revision March 30 2008 
...
int tree_sampling(char* frags,char* sol,int fraction)
{
        ...
        // to be of the same order as the # of flips performed 
								...
								float rd1 = drand48();
								int maxiter=1000000; if (EXAMPLE ==1) maxiter = 100000000;
								...
								//																if (clist[k].phased > 50) continue;
																								...
																								if ( (iter%2000==0 && iter > 0 && iter < 3000 ))// || iter > 10000 && iter%10000==0)
																								{ 
...
//																																if (clist[k].MEC == MF && EXAMPLE ==1) { fprintf(stdout,"sampled best haplotype iter %d \n",iter);} // exit(0);}
	...
	//																															if (EXAMPLE ==1) { for (i=0;i<snps;i++) fprintf(stderr,"%c",h1[i]); if (h1[snps/2] == h1[snps/2-1] && clist[k].MEC == MF ) fprintf(stderr," H1\n"); else if (clist[k].MEC == MF) fprintf(stderr," H2\n"); else fprintf(stderr,"\n");} 
																...
																if ((iter%10 ==0 && GIBBS ==1 && EXAMPLE ==0))
																{
																								...
																								if (STDERR) fprintf(stderr,"MCMC %6d %d %d %f BESTMEC %d Log-lh %f bestLL %f time %s \n",iter,miscalls,calls,(double)miscalls/(double)calls,bestscore_mec,mcmc_ll,mcmcbest_ll,buf);
																								fprintf(stdout,"FULLMCMC %6d %d %d %f BESTMEC %d Log-lh %f bestLL %f time %s\n",iter,miscalls,calls,(double)miscalls/(double)calls,bestscore_mec,mcmc_ll,mcmcbest_ll,buf);
